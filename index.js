...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  bot.sendMessage(chatId,
    ...
    `/pay — send money to user or number\n` +
    `/request — request payment\n` +
    `/balance — check balance\n` +
    `/history — recent transactions\n` +
    `/connect — add bot to your channel\n` +
    `/post — post paywall to channel`,
    { parse_mode: 'Markdown' });
});
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('reqpay:')) {
    ...
    try {
      ...
      if (result.status === 'SUCCESSFUL') {
        updateTx(ref, 'successful');
        bot.sendMessage(chatId, `✅ *Paid!* ${fmt(amount)} sent successfully.`, { parse_mode: 'Markdown' });
